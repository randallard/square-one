...
| **Hand availability / last hand used** | Which hand finished the last action; current handhold (mini-wave hand, couple hold, star grip…) | The alternating-hand comfort rule (right-hand action comfortably follows left); styling; Taminations tracks a `hands` value per movement for exactly this |
...
| **Flow quality** | Body-flow compatibility call-to-call: no abrupt reversals, no overflow (cumulative same-direction turning), good/bad preceding-following pairs (CALLERLAB call analysis lists these) | "Technically legal but awkward" is the #1 novice-caller failure — a perfect drill-game metric |
| **Hand alternation** | Consecutive actions alternate hands where possible | Same — comfort scoring |
